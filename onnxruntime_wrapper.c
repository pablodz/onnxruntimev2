...
IONames GetIONames(const OrtSession *session)
{
  ...
  for (int i = 0; i < input_count; i++)
  {
    ...
    input_types[i] = (char *)malloc((strlen("complex128") + 1) * sizeof(char));
  ...
  for (int i = 0; i < output_count; i++)
  {
    ...
    output_types[i] = (char *)malloc((strlen("complex128") + 1) * sizeof(char));
                         ...
                         OrtEnv *env, OrtSession **out)
{
  ...
  status = ort_api->CreateSessionFromArray(env, model_data, model_data_length, options, out);
