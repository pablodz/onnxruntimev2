...
func NewSessionWithONNXData[IT TensorData, OT TensorData](onnxData []byte, inputNames,
	outputNames []string, inputs []*Tensor[IT], outputs []*Tensor[OT],
) (*Session, error) {
	if !IsInitialized() {
		return nil, NotInitializedError
	}
...
func NewSessionONNXDataWithType(onnxData []byte, inputNames,
	outputNames []string, inputs []*TensorWithType, outputs []*TensorWithType,
) (*Session, error) {
	if !IsInitialized() {
		return nil, NotInitializedError
	}
...
func NewSessionWithPathWithTypeWithCUDA(path string, inputNames,
	outputNames []string, inputs []*TensorWithType, outputs []*TensorWithType, opts ...string,
) (*Session, error) {
	if !IsInitialized() {
		return nil, NotInitializedError
	}
...
// The same as NewSession, but takes a slice of bytes containing the .onnx
// network rather than a file path.
func NewSessionWithPath(path string, inputNames,
	outputNames []string, inputs []*Tensors, outputs []*Tensors,
) (*Session, error) {
	if !IsInitialized() {
		return nil, NotInitializedError
	}
	...
	for i, v := range inputs {
		...
		case v.TensorInt64 != nil:
			inputOrtTensors[i] = v.TensorInt64.ortValue
		}
	}
	for i, v := range outputs {
		...
		case v.TensorInt64 != nil:
			outputOrtTensors[i] = v.TensorInt64.ortValue
		}
	}
	return &Session{
		ortSession:  ortSession,
...
func NewSession(onnxFilePath string, inputNames, outputNames []string, inputs []*Tensors, outputs []*Tensors) (*Session, error) {
